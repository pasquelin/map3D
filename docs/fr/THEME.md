...
| `colors.cluster.stroke` | Contour clair des parts du donut (épaisseur `clusters.strokeWidth`). | `'#ffffff'` |
...
| `colors.readable.dark` | Texte posé sur une couleur arbitraire (pastille d'affiliation) : encre sombre quand le fond est clair. | `'#101828'` |
| `colors.readable.light` | Encre claire quand le fond est sombre. | `'#ffffff'` |
| `colors.readable.threshold` | Luminance perçue au-delà de laquelle le fond est jugé clair. | `0.6` |
| `colors.pinShade.color` | Assombrissement du bas d'une pastille de dock à couleur explicite (`color-mix`). | `'#000000'` |
| `colors.pinShade.percent` | Part (%) de `pinShade.color` dans le mélange. | `45` |
...
| `markers.selectedGapPx` | Écart (px, chaque côté) entre le marker et son anneau de sélection. | `4` |
...
| `markers.selectionRingExtraPx` | Surplus de diamètre (px) de l'anneau de multi-sélection d'un sprite — repli de `<MarkerLayer selectionRing>`. | `4` |
| `markers.tipGapPx` | Écart (px) entre le haut du visuel et son infobulle. | `10` |
| `markers.ringColor` | Anneau clair entre corps et halo du marker par défaut. | `'#ffffff'` |
| `markers.glossColor` | Couleur du reflet. | `'#ffffff'` |

## `clusters` — Géométrie du cluster par défaut (donut)

| Clé | Description | Défaut |
| ------------------------ | -------------------------------------------------------------------------- | ------------------- |
| `clusters.coreRadius` | Rayon du cœur (px) selon le nombre total de points. | _(fonction)_ |
...
| `clusters.sizeRatio` | Diamètre d'une pastille custom en fraction de `markers.size` — repli de `<ClusterSurface size>`. | `1.18` |
| `clusters.text.segmentSize` | Chiffres du donut : taille (px) du compte d'une part. | `13` |
| `clusters.text.segmentSizeWide` | Taille (px) d'une part à partir de `wideFrom`. | `11` |
| `clusters.text.coreSize` | Taille (px) du total au centre. | `19` |
| `clusters.text.coreSizeWide` | Taille (px) du total à partir de `wideFrom`. | `16` |
| `clusters.text.wideFrom` | Nombre de points à partir duquel les tailles « wide » s'appliquent. | `100` |
| `clusters.text.weight` | Graisse des chiffres. | `800` |
| `clusters.tip.background` | Vignette de survol d'une part (portail hors `.m3d-root`, lue en JS) ; police = `typography.fontFamily`. Fond. | `'rgba(17,24,39,0.96)'` |
| `clusters.tip.color` | Texte de la vignette. | `'#ffffff'` |
| `clusters.tip.fontSize` | Taille (px) du texte. | `12` |
| `clusters.tip.weight` | Graisse du texte. | `600` |
| `clusters.tip.radius` | Rayon d'angle (px). | `8` |
| `clusters.tip.gap` | Écart (px) entre la part et la vignette. | `7` |
| `clusters.tip.paddingY` | Marge interne verticale (px). | `6` |
| `clusters.tip.paddingX` | Marge interne horizontale (px). | `10` |
| `clusters.tip.dotSize` | Diamètre (px) de la pastille de couleur. | `9` |
| `clusters.tip.shadow` | Ombre portée. | `'0 6px 20px rgba(0,0,0,0.35)'` |
| `clusters.tip.separatorOpacity` | Opacité du séparateur. | `0.6` |
...
| `animations.flyEasing` | Courbe d'accélération des vols caméra. | _(fonction)_ |
...
`themeToVars` publie aussi ces rythmes en variables CSS : `--m3d-{pulse,halo,bob,enter,cluster-enter}-{dur,ease,scale,amp,stagger}` et `--m3d-menu-ease`. Une spec à `false` donne une durée `0ms`.
...
| `spacing.rowMenuGap` | Écart entre le bouton « … » d'une ligne et son menu. | `2` |
...
| `sizing.rowMenuW` | Largeur (px) du menu d'actions d'une ligne de liste. | `180` |
| `sizing.pinSize` | Taille (px) d'une pastille du dock — repli de `<PinnedDock size>`. | `64` |
| `sizing.panelMaxHeight.searchScope` | Hauteur maximale (px) du menu de portée de la recherche. | `280` |
...
| Clé | Description | Défaut |
|---|---|---|
| `relations.width` | Épaisseur du trait des liens (px écran). | `8` |
| `relations.defaultColor` | Dernier repli de couleur d'un lien. | `'#ffd400'` |
| `relations.routeColor` | Couleur de l'itinéraire réel. | `'#7c4dff'` |
| `relations.hoverDarken` | Facteur d'assombrissement du trait survolé. | `0.72` |
| `relations.hubRadius` | Rayon (px écran) du socle sous le marker source. | `26` |
| `relations.casingWidth` | Contour sombre sous le trait (px). | `3` |
| `relations.minOpacity` | Opacité du lien le moins bien classé. | `1` |
| `relations.dash` | Pointillé défilant des traits de recherche (`DashStyle`). | `{ length: 20, gap: 8, speed: 16, gapOpacity: 0.3 }` |

## `draw` — Style d'une forme nouvellement dessinée

| Clé | Description | Défaut |
|---|---|---|
| `draw.width` | Épaisseur du trait. | `8` |
| `draw.fillOpacity` | Opacité du remplissage. | `0.3` |
| `draw.stroke` | Style de trait (`StrokeStyle`). | `'solid'` |
| `draw.strokeOpacity` | Opacité du trait. | `0.95` |

## `path` et `zone` — Tracés et zones drapés

| Clé | Description | Défaut |
|---|---|---|
| `path.width` | `<PathLayer>` : largeur du tracé (m au sol). | `6` |
| `path.casingWidth` | `<PathLayer>` : contour (m au sol). | `3` |
| `zone.width` | `<ShapeLayer>` : largeur du contour (m au sol). | `6` |
| `zone.fillOpacity` | `<ShapeLayer>` : opacité du remplissage. | `0.22` |
...
| `tiles.filter.invert` | `0` = inchangé ; `1` inverse — spectaculaire mais rarement lisible. Optionnel. | _(non défini)_ |
| `tiles.filter.hueRotate` | Rotation de teinte, en degrés. Optionnel. | _(non défini)_ |
