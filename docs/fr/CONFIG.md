...
| `providers.buildings.mountPerFrame` | Tuiles montées dans la scène par frame au plus. ⚠️ 1 → **2**. Le montage était dominé par la construction de l'arbre de collision (~41 ms par tuile dense, 97 % du coût), désormais faite côté worker : il ne reste que la pose de l'arbre (~0,05 ms), les couleurs développées (~1 ms) et l'upload GPU — non mesuré, d'où un doublement plutôt qu'une ouverture en grand. | `2` |
| `providers.buildings.maxInflight` | Tuiles simultanément en cours de téléchargement **et d'extrusion**. ⚠️ 2 → **4**, aligné sur `workerPoolSize` : laissé à 2, il aurait affamé le pool, dont deux workers sur quatre seraient restés oisifs. | `4` |
| `providers.buildings.workerPoolSize` | Workers d'extrusion tournant en parallèle. ⚠️ Nouveau, pendant obligé de l'arbre de collision construit côté worker : une tuile dense y coûte ~60 ms au lieu de ~19, et un fil unique les sérialiserait. Mesuré sur 24 tuiles z14 parisiennes — 1430 ms à un worker, 587 ms à trois, 559 ms à quatre, puis plus rien et une **régression** à huit. Le pool se borne au nombre de cœurs moins un. Inutile au-delà de `maxInflight`. | `4` |
...
| Clé | Description | Défaut |
|---|---|---|
| `style.renderOrder.shapes` | Formes (`<ShapeLayer>`). <!-- audit: à vérifier à la fusion (cœur) --> | `1` |
| `style.renderOrder.paths` | Tracés (`<PathLayer>`). <!-- audit: à vérifier à la fusion (cœur) --> | `1` |
| `style.renderOrder.links` | Liens. <!-- audit: à vérifier à la fusion (cœur) --> | `1` |
| `style.renderOrder.relations` | Relations — au-dessus des formes, tracés et liens. <!-- audit: à vérifier à la fusion (cœur) --> | `2` |
| `style.renderOrder.drawings` | Dessins — au-dessus de tout le reste. <!-- audit: à vérifier à la fusion (cœur) --> | `4` |
...
## `capture` — Capture d'image de la carte

Défauts de la capture (« Prendre une photo » et `engine.capture()`). Ne contient que des valeurs **sérialisables** : le rasteriseur d'overlay et les callbacks (mail/trace) sont des fonctions, passées en prop `<Map capture>` — cf. [PROPS.md](PROPS.md).

| Clé | Description | Défaut |
|---|---|---|
| `capture.format` | Format par défaut de l'image produite (`'png' | 'jpeg' | 'webp'`). | `'png'` |
