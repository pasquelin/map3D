...
`undo`/`redo`/`canUndo`/`canRedo`, `clear`/`canErase`, `settings` (+ `useDrawSettings()`),
...
`compass`, `layers`, `target`, `pedestrian`, `zoom`, `fullscreen`. The `compass` group gathers the whole point of view
