...
| `globe.buildingHoverColor` | Tint of a hovered building while the selection tool is active. It REPLACES the footprint's vertex colours — shading included, which is baked into them: that is what makes it stand out of a whole neighbourhood. | `'#F2B441'` |
