...
| `providers.tiles.errorTtlMs` | Duration (ms) a tile that exhausted its attempts stays in error before being requested again, if it is still in view. `0` = permanent error. | `30000` |
| `providers.tiles.staleFrames` | Frames out of view beyond which a pending tile leaves the download queue. Seen again, it comes back. | `120` |
| `providers.tiles.fetch.timeoutMs` | Network policy (`FetchPolicy`) of the Google **session creation**. Give up on a request with no response. `0` = no limit. | `10000` |
| `providers.tiles.fetch.retries` | Retries after a network failure or 5xx. `0` = none. | `0` |
| `providers.tiles.fetch.backoffMs` | Wait before the first retry, doubled on each round, with a random share. `0` = immediate retry. | `0` |
...
| `providers.buildings.errorTtlMs` | Duration (ms) a tile that exhausted its attempts stays in error before being requested again, if it is still in view. `0` = permanent error. | `30000` |
| `providers.buildings.staleFrames` | Frames out of view beyond which a pending tile leaves the download queue. Seen again, it comes back. | `120` |
...
| `interaction.lockFlashMs` | Duration (ms) of the “shape locked” flash when a gesture targets a shape its constraints forbid editing. | `800` |
...
| `performance.shapeGroundSamples` | Points of a zone's outline probed to find the lowest ground under its volume — the base of an extrusion does not float on a slope. | `16` |
...
| Key | Description | Default |
|---|---|---|
| `style.renderOrder.shapes` | Shapes (`<ShapeLayer>`). | `1` |
| `style.renderOrder.paths` | Paths (`<PathLayer>`). | `1` |
| `style.renderOrder.links` | Links. | `1` |
| `style.renderOrder.relations` | Relations — above shapes, paths and links. | `2` |
| `style.renderOrder.drawings` | Drawings — above everything else. | `4` |
...
| `clustering.levelQuantization` | Zoom quantisation for the stability of cluster steps: the zoom is rounded to a multiple of this value before the step is chosen. | `1` |
