...
| `providers.buildings.maxViewDistance` | Maximum reach (m) of the extent served to buildings; past it the raster basemap stands alone. ⚠️ This is what makes coverage **continuous** in tilted view. Unbounded, the extent came from a ray grid whose rays crossing the horizon were discarded: sawtooth reach between 2.8 and 36.3 km, two abrupt collapses (59°, 74°), and 8 to 1058 tiles requested at constant altitude. Raising the reach costs RAM (peak grows as n²: 24 tiles at 6 km, 40 at 8 km). | `6000` |
...
| `providers.buildings.maxRequest` | Budget of tiles requested for one view. ⚠️ Now only a **safety net**: `maxViewDistance` bounds the reach, and the measured peak at 6 km is 24 tiles. This budget used to trigger a fallback to a fixed-size square around the looked-at point whenever exceeded — hence an abrupt switch between two regimes in tilted view. See [TILES.md § 5](TILES.md). | `49` |
...
| `providers.tiles3d.hideVolumeWhenClamped` | Hides the internal buildings above `providers.buildings.maxViewAltitude`: from higher up they cover only a few pixels and leave a "square" in the void. Faded out, hidden, frozen AND **destroyed** (RAM/VRAM returned, reloaded on the way back). The criterion is a height above ground, so **valid at any tilt**. **The mode does not change** (stays `'3d'`). `false` = always shown. Internal only. | `true` |
